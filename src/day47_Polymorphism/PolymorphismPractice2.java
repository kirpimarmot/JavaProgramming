...
public class PolymorphismPractice2 {
    ...
    public static void main(String[] args) {
        ...
        List<Employee> scrumMembers = new ArrayList<>();// tum Tester ve Developer lari icine koyacagimiz yeni Arraysist

        for (Employee employee : employees) {//yeni listeye hepsini koyabilmek icin foreach loop ile cagiriyoruz
            if (employee instanceof Tester || employee instanceof Developer){
                scrumMembers.add(employee);//ve yeni arrayslist'e ekliyoruz
            }
        }
        ...
        Tester hasTheMaxSalary = testers.get(0);
        for (Tester tester : testers) {
                if (tester.getSalary()>hasTheMaxSalary.getSalary()){
                    hasTheMaxSalary=tester;
                }

            }

            System.out.println("Tester Has the max salary is: "+hasTheMaxSalary);


        //2.5 which developer has the maximum salary?
        Developer DeveloperHasTheMaxSalary = developers.get(0);
        for (Developer developer : developers) {
            if (developer.getSalary()>DeveloperHasTheMaxSalary.getSalary()){
                DeveloperHasTheMaxSalary=developer;
            }
        }
        System.out.println("Developer has the max salary is: "+DeveloperHasTheMaxSalary);
        }
